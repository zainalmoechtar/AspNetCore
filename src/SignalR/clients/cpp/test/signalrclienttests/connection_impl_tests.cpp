...
TEST(connection_impl_start, cannot_start_non_disconnected_exception)
{
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)>& callback) { callback(nullptr, "{ }\x1e"); });
...
TEST(connection_impl_start, connection_state_is_connecting_when_connection_is_being_started)
{
    ...
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)>& callback) { callback(std::make_exception_ptr(std::runtime_error("should not be invoked")), ""); },
        /* send function */ [](const std::string &, const std::function<void(const std::exception_ptr&)> & callback){ callback(std::make_exception_ptr(std::runtime_error("should not be invoked")));  },
        /* connect function */[](const std::string &, const std::function<void(const std::exception_ptr&)> & callback)
        {
...
TEST(connection_impl_start, start_sets_id_query_string)
{
    ...
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)> & callback) { callback(std::make_exception_ptr(std::runtime_error("should not be invoked")), ""); },
        /* send function */ [](const std::string &, const std::function<void(const std::exception_ptr&)> & callback) { callback(std::make_exception_ptr(std::runtime_error("should not be invoked")));  },
        /* connect function */[&query_string](const std::string& url, const std::function<void(const std::exception_ptr&)> & callback)
    {
...
TEST(connection_impl_send, send_fails_if_transport_fails_when_receiving_messages)
{
    ...
    auto websocket_client = create_test_websocket_client([](const std::function<void(const std::exception_ptr&, const std::string&)>& callback) { callback(nullptr, ""); },
        /* send function */ [](const std::string &, const std::function<void(const std::exception_ptr&)>& callback)
        {
...
TEST(connection_impl_start, start_fails_if_negotiate_request_fails)
{
    ...
    websocket_client->set_receive_function([](const std::function<void(const std::exception_ptr&, const std::string&)>& completion_callback)
    {
        completion_callback(nullptr, "{ }\x1e");
    });
...
TEST(connection_impl_start, start_fails_if_negotiate_response_has_error)
{
    ...
    auto websocket_client = std::make_shared<test_websocket_client>();
    websocket_client->set_connect_function([tce](const std::string&, const std::function<void(const std::exception_ptr&)>& completion_callback) mutable
    {
        try
        {
...
TEST(connection_impl_start, negotiate_follows_redirect)
{
    ...
    websocket_client->set_connect_function([&connectUrl](const std::string& url, const std::function<void(const std::exception_ptr&)> & completion_callback) mutable
    {
        connectUrl = url;
        completion_callback(nullptr);
    });
...
TEST(connection_impl_process_response, process_response_logs_messages)
{
    ...
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [wait_receive](const std::function<void(const std::exception_ptr&, const std::string&)> & callback)
        {
            wait_receive->set();
            callback(nullptr, "{ }");
        });
...
TEST(connection_impl_send, message_sent)
{
    ...
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)> & callback) { callback(nullptr, "{ }\x1e"); },
        /* send function */ [&actual_message](const std::string & message, const std::function<void(const std::exception_ptr&)> & callback)
    {
...
TEST(connection_impl_send, exceptions_from_send_logged_and_propagated)
{
    std::shared_ptr<log_writer> writer(std::make_shared<memory_log_writer>());
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)> & callback)
        {
            callback(nullptr, "{}");
        },
        /* send function */ [](const std::string &, const std::function<void(const std::exception_ptr&)> & callback)
...
TEST(connection_impl_set_message_received, callback_invoked_when_message_received)
{
    int call_number = -1;
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [call_number](const std::function<void(const std::exception_ptr&, const std::string&)> & callback)
        mutable {
        ...
        callback(nullptr, responses[call_number]);
    });
...
TEST(connection_impl_stop, stopping_disconnecting_connection_returns_cancelled_task)
{
    ...
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)> & callback) { callback(nullptr, "{ }\x1e"); },
        /* send function */ [](const std::string &, const std::function<void(const std::exception_ptr&)> & callback){ callback(std::make_exception_ptr(std::runtime_error("should not be invoked"))); },
        /* connect function */ [&close_event](const std::string&, const std::function<void(const std::exception_ptr&)> & callback) { callback(nullptr); },
        /* close function */ [&close_event](const std::function<void(const std::exception_ptr&)> & callback)
        {
            close_event.wait();
            callback(nullptr);
        });
...
TEST(connection_impl_stop, can_start_and_stop_connection_multiple_times)
{
    ...
    {
        auto websocket_client = create_test_websocket_client(
            /* receive function */ [](const std::function<void(const std::exception_ptr&, const std::string&)> & callback) { callback(nullptr, std::string("{ }\x1e")); });
        auto connection = create_connection(websocket_client, writer, trace_level::state_changes);
...
TEST(connection_impl_stop, stop_cancels_ongoing_start_request)
{
    ...
    auto websocket_client = create_test_websocket_client(
        /* receive function */ [disconnect_completed_event](const std::function<void(const std::exception_ptr&, const std::string&)> & callback)
        {
...
TEST(connection_impl_stop, ongoing_start_request_canceled_if_connection_stopped_before_init_message_received)
{
    ...
    auto websocket_client = create_test_websocket_client(/*receive function*/ [](const std::function<void(const std::exception_ptr&, const std::string&)> & callback)
    {
        callback(nullptr, "{}");
    });
